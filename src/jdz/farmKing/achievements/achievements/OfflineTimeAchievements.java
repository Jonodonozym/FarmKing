...
public class OfflineTimeAchievements extends FarmAchievementSeries {
	private static final long[] requiredMinutes = new long[] { 60, 240, 480, 720, 1440, 2880, 7200 };

	public OfflineTimeAchievements() {
		super("Omniscient", getAchievements());
	}

	private static List<Achievement> getAchievements() {
		List<Achievement> achievements = new ArrayList<Achievement>();
		for (int i = 0; i < requiredMinutes.length; i++) {
			Achievement achievement = new StatAchievement("Omniscient " + RomanNumber.of(i), FarmStats.OFFLINE_TIME,
					requiredMinutes[i] * 60, Material.WATCH, (short) 0,
					"Be offline for " + timeFromMinutes(requiredMinutes[i]) + " in a single game");
