...
public class Crop {
	private static final int maxCMI = 5;
	private static double[] buy10Mult = new double[maxCMI], buy100Mult = new double[maxCMI], buy1000Mult = new double[maxCMI];
	private static final List<Map<Integer,Double>> buyMap = new ArrayList<Map<Integer, Double>>();
	public int CMI = 0; 
	...
	public Crop(Farm farm, CropType cropType, Location location, int level, int quantity, int isGenerated){
		this.farm = farm;
		...
		this.world = Main.plugin.getServer().getWorlds().get(0);
	...
	public Crop(Farm farm, CropType cropType, Location location){
		this(farm, cropType, location, 0, 0, 0);
	}
	...
	public void updateSign(){
		...
		if (sign.getType() == Material.WALL_SIGN){
			Sign s = (Sign) sign.getState();
			double inc1 = cropType.baseIncome * upgradeBonus * farm.globalIncomeMultiplier * incomeBonus;
	...
	public void updatePrice(){
		...
		while (quantity > buyMap.get(CMI).size())
			buyMap.get(CMI).put(buyMap.get(CMI).size(), Double.parseDouble(String.format("%10.9f", buyMap.get(CMI).get(buyMap.get(CMI).size()-1) * cm)));

		cost = buyMap.get(CMI).get(quantity) * cropType.basePrice;
	...
	public boolean clickItemFrame(ItemFrame itemFrame, Location cropLocation){
		...
		double upgradePrice = buyMap.get(CMI).get(CropType.upgradeCostfromPrice.get(upgradeLevel-1)) * cropType.basePrice;
	...
	private void spawnNextUpgrade(Location l){
		if (level < 10){
			...
			try { nextUpgradeFrame = world.spawn(l, ItemFrame.class); }
			catch (IllegalArgumentException e) { nextUpgradeFrame = (ItemFrame)world.getNearbyEntities(l, 1, 1, 1).iterator().next(); }
			ItemStack i = new ItemStack(Material.WOOL, 1, (short)14);
			int nextQuan = CropType.upgradeQuantityRequirement.get(level);
			String nextCost = UPEconomyAPI.charFormat( buyMap.get(CMI).get(CropType.upgradeCostfromPrice.get(level))*cropType.basePrice, 4);
	 ...
	 */
	public static Crop fromString(Farm f, CropType cropType, Location location, String s){
		String[] args = s.split(",");
		Crop c = new Crop(f, cropType, location, Integer.parseInt(args[0]), Integer.parseInt(args[1]), Integer.parseInt(args[2]));
		return c;
	}
